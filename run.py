...
def clear_log_dir(air):
    ...
    """
    log = os.path.join(os.getcwd(), '../air', air, 'log')
...
def get_path(content,device=None,air="openCard.air"):
    ...
    if content=="result":
        ...
        log_dir = os.path.join(root_path, '../air', air, 'log', device.replace(".", "_").replace(':', '_'))
        #如果没有日志路径则创建一个
        ...
        #返回测试用例路径
        path = os.path.join(root_path, '../air', air)
    else:
        #返回根目录
        path = root_path
    return path
...
def get_cases():
    root_path = os.getcwd()
    cases=[]
    for name in os.listdir(os.path.join(root_path, '../air')):  # 遍历当前路径下的文件夹和文件名称
